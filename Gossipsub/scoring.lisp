...
;; TODO : make valid twp cons

(definecd is-valid-twp-help (twp :twp) :boolean
  (v (endp twp)
     (let ((params (cddar twp)))
       (^ (weightsp (cadar twp))
          ...
          (is-valid-twp-help (cdr twp))))))

(definecd is-valid-twp (twp :twp) :boolean
  (^ (consp twp)
     (is-valid-twp-help twp)))
...
(property lookup-twpm-check2 (top :topic twpm :twp)
  ...
  :hints (("goal" :in-theory (enable is-valid-twp is-valid-twp-help mget))))
...
(property car-twpm-check5 (twpm :twp :cons params :all)
  ...
  :hints (("Goal" :in-theory (enable is-valid-twp is-valid-twp-help)))
  :rule-classes :forward-chaining)
...
(property nonpos-third-weight (wt :weights)
     (>= 0 (mget :w3 wt))
     :hints (("Goal" :in-theory (enable weightsp)))
     :rule-classes :linear)
...
;; 3.1. Decreasing meshMessageDeliveries below the threshold decreases calcP3 score
(property dec-calcP3-lt-thres
  (meshTime :non-neg-rational
            activationWindow :nat
            mmd1 mmd2 :non-neg-rational
            w3 :neg-rational
            meshMessageDeliveriesCap 
            meshMessageDeliveriesThreshold :pos-rational)
  (=> (^ (> meshTime activationWindow)
         (<= MESHMESSAGEDELIVERIESTHRESHOLD
             MESHMESSAGEDELIVERIESCAP)
         (< mmd2 meshMessageDeliveriesThreshold)
         (> mmd1 meshMessageDeliveriesThreshold))
      (< (* w3
            (calcP3 meshTime activationWindow mmd2
                    meshMessageDeliveriesCap
                    meshMessageDeliveriesThreshold))
         (* w3
            (calcP3 meshTime activationWindow mmd1
                    meshMessageDeliveriesCap
                    meshMessageDeliveriesThreshold))))
  :hints (("Goal" :in-theory (enable calcP3 calc-deficit))))


;; 4.1. Decreasing meshMessageDeliveries below the threshold decreases calcP3 score
(property dec-calcP3b-lt-thres
  (meshTime :non-neg-rational
            activationWindow :nat
            meshFailurePenalty mmd1 mmd2 :non-neg-rational
            w3b :neg-rational
            meshMessageDeliveriesCap 
            meshMessageDeliveriesThreshold :pos-rational)
  (=> (^ (> meshTime activationWindow)
         ...
         (> mmd1 meshMessageDeliveriesThreshold))
      (< (* w3b
            (calcP3b meshTime activationWindow meshFailurePenalty mmd2
                     meshMessageDeliveriesCap
                     meshMessageDeliveriesThreshold))
         (* w3b
            (calcP3b meshTime activationWindow meshFailurePenalty mmd1
                     meshMessageDeliveriesCap
                     meshMessageDeliveriesThreshold))))
  :hints (("Goal" :in-theory (enable calcP3b calc-deficit))))


(property dec-calcP3-p3b-sum
  (meshTime :non-neg-rational
            activationWindow :nat
            meshFailurePenalty mmd1 mmd2 :non-neg-rational
            w3 w3b :neg-rational
...
(property mmd-dec-score
  (imd mmd1 mmd2 mt fmd mfp p :non-neg-rational wtpm :wp)
  :check-contracts? nil
  :h (^ (<= (params-meshMessageDeliveriesThreshold (cdr wtpm))
            (params-meshMessageDeliveriesCap (cdr wtpm)))
        (> mt (mget :activationwindow (cdr wtpm)))
        (< mmd2 (params-meshMessageDeliveriesThreshold (cdr wtpm)))
        (> mmd1 (params-meshMessageDeliveriesThreshold (cdr wtpm))))
  (>= (calcScoreTopic (tctrs imd mmd1  mt fmd mfp) wtpm)
      (calcScoreTopic (tctrs imd mmd2  mt fmd mfp) wtpm))
  :instructions
  (:pro
   (:claim (^ (non-neg-rationalp (mget :w1 (car wtpm)))
              (non-neg-rationalp (mget :w2 (car wtpm)))
              (neg-rationalp (mget :w3 (car wtpm)))
              (neg-rationalp (mget :w3b (car wtpm)))
              (neg-rationalp (mget :w4 (car wtpm)))))
   (:dv 1) (:r 2) :s :top
   (:dv 2) (:r 2) :s :top :s
   (:claim
    (< (+ (* (mget :w3 (car wtpm))
              (calcp3 mt (mget :activationwindow (cdr wtpm))
                      mmd2
                      (mget :meshmessagedeliveriescap (cdr wtpm))
                      (mget :meshmessagedeliveriesthreshold (cdr wtpm))))
           (* (mget :w3b (car wtpm))
              (calcp3b mt (mget :activationwindow (cdr wtpm))
                       mfp mmd2
                       (mget :meshmessagedeliveriescap (cdr wtpm))
                       (mget :meshmessagedeliveriesthreshold (cdr wtpm)))))
        (+ (* (mget :w3 (car wtpm))
           ...
           (* (mget :w3b (car wtpm))
              (calcp3b mt (mget :activationwindow (cdr wtpm))
                       ...
                       (mget :meshmessagedeliveriescap (cdr wtpm))
                       (mget :meshmessagedeliveriesthreshold (cdr wtpm))))))
    :hints (("Goal" :use ((:instance dec-calcP3-p3b-sum
                                     (w3 (mget :w3 (car wtpm)))
                                     (w3b (mget :w3b (car wtpm)))
                                     (meshTime mt)
                                     (activationWindow (mget :activationwindow
                                                             (cdr wtpm)))
                                     (meshMessageDeliveriesCap
                                      (mget :meshmessagedeliveriescap (cdr
                                                                       wtpm)))
                                     (meshMessageDeliveriesThreshold
                                      (mget :meshmessagedeliveriesthreshold
                                            (cdr wtpm)))
                                     (meshFailurePenalty mfp))))))
   (:claim (not (<
                 (+ (* (mget :w3 (car wtpm))
                       (calcp3 mt (mget :activationwindow (cdr wtpm))
                               mmd1
...
(property mmd-dec-score2
  (imd mmd1 mmd2 mt fmd mfp p :non-neg-rational wtpm :wp)
  :check-contracts? nil
  :h (^ (<= (params-meshMessageDeliveriesThreshold (cdr wtpm))
            (params-meshMessageDeliveriesCap (cdr wtpm)))
        (> mt (mget :activationwindow (cdr wtpm)))
        (> (mget :topicweight (cdr wtpm)) 0)
        (< mmd2 (params-meshMessageDeliveriesThreshold (cdr wtpm)))
        (> mmd1 (params-meshMessageDeliveriesThreshold (cdr wtpm))))
  (> (calcScoreTopic (tctrs imd mmd1 mt fmd mfp) wtpm)
     (calcScoreTopic (tctrs imd mmd2 mt fmd mfp) wtpm))
