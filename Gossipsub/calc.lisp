...
(b* ((TOP 'SUB2)
     ...
     (PTC '(((a . agg)
             (:0tag . tctrs)
             (:firstmessagedeliveries . 50)
             (:invalidmessagedeliveries . 0)
             (:meshfailurepenalty . 0)
             (:meshmessagedeliveries . 50)
             (:meshtime . 100))
            ((a . blocks)
             (:0tag . tctrs)
             (:firstmessagedeliveries . 50)
            ...
            ((a . sub2)
             ...
             (:meshtime . 100)))))
  (list
   (cons top (calcScoreTopic (lookup-tctrs p top ptc)
                              (mget top *ETH-TWP*)))
   (cons 'TOTAL (lookup-score p (calc-nbr-scores-map ptc pcm *eth-twp*))))
  )


;; -100 if stopped completely in a topic
-75019/750*3 = -300


42.5 for the ACC and BLOCKS.

42.5 + 10.88*t/T < 300*i/T


Generalize this fmls.


do you have any distribution? of how many topics each node subscribes to?
What happenss when some node is constantly dropping messages?
